...
void loop(){
  ...
  if (Firebase.getInt(firebaseData, "/home1/Camera/Photo")) {
  ...
  if (Firebase.getInt(firebaseData, "/home1/Camera/PIR")) {
    if (firebaseData.dataType() == "int") {
      Serial.print("PIR = ");
      Serial.println(firebaseData.intData());
  ...
  if(flag == 1) {
      if(motionDetected){ 
        int PIRStat = 1; 
        Firebase.setInt(firebaseData, "/home1/Camera/PIRStat",PIRStat); 
        sendPhotoTelegram();
