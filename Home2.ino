// Line config
//6HxHdNubzHq9YhsmJEZsEVncoutgedaF20WcOCmbjqP บูม
...
int smoke_firebase = 0; //ควัน Home 1
int flame_firebase = 0; //เปลวไฟ Home 1
int PIR_firebase = 0;
int Lamp = 22;
int Fan = 19;
int water = 4;
...
void setup(){
  ...
  LINE.setToken(LINE_TOKEN); 
    if (Firebase.getString(firebaseData, "/home2/Location/Latitude")) {
      if (firebaseData.dataType() == "string") {
       la = firebaseData.stringData();
      }
    }
    if (Firebase.getString(firebaseData, "/home2/Location/Longitude")) {
      if (firebaseData.dataType() == "string") {
    ...
    Firebase.setInt(firebaseData, "/home2/Camera/PIR",flag);
    Firebase.setInt(firebaseData, "/home2/DOORSTAT",door1stat);
    check2 = 1;      
    Firebase.setInt(firebaseData, "/home2/Token/check",check2);
    checkB = 1;     
    Firebase.setInt(firebaseData, "/home2/Token/checkB",checkB);
...
void loop(){
  GetTime();
  Firebase.setInt(firebaseData, "/home2/Token/Time",currentMinute);
  ...
  if(PIRStat == 1){
    s4 = "  🆗";
    if (Firebase.getInt(firebaseData, "/home2/Camera/PIRStat")) {
    ...
    if (PIR_firebase == 1 ) {
      ...
      if (Line_stage == 1){
        message = "บ้านเลขที่ 493 ตรวจพบการเคลื่อนไหว ในขณะที่เจ้าของบ้านไม่อยู่ www.google.com/maps/place/" + la + "," + lon ;
        GetLine();
        }               
  ...
  if(doortelegram == 1)   { 
    ...
    if(door1stat == 0 && doorLockMonitor == 0) {                              //door1stat = 0   และ doorLockMonitor = 0
      ...
      digitalWrite(Lamp, LOW);
      if (Line_stage == 1){
        message = "บ้านเลขที่ 493 ตรวจพบการงัดแงะประตู หน้าต่างที่ www.google.com/maps/place/" + la + "," + lon ;
        GetLine();
        }        
  ...
  Firebase.setInt(firebaseData, "/home2/SMOKE",smoke);
  delay(100);
    if (smoke > 749) {  //แก๊ส       
    digitalWrite(buzzer,sound);
    if (sound == LOW){
    ...
    if (smoke > 749 && Fansmoke == 0 ) {
      ...
      if (Line_stage == 1){
        message = "บ้านเลขที่ 493 ตรวจพบแก๊สรั่วไหลภายในบริเวณ www.google.com/maps/place/" + la + "," + lon ;
        GetLine();
        }      
      s1 = "  🚨";
    ...
    Fansmoke = 0;
      if (Line_stage == 1){
        message = "บ้านเลขที่ 493 ค่าแก๊สกลับสู่สภาวะปกติ www.google.com/maps/place/" + la + "," + lon ;
        GetLine();
        }    
      }        
  }
  ...
  Firebase.setInt(firebaseData, "/home2/FLAME",flame);
  delay(100);
    ...
    if (flame == 0 && flamewater == 0) {
      ...
      if (Line_stage == 1){
        message = "บ้านเลขที่ 493 ตรวจพบเปลวไฟภายในบริเวณ www.google.com/maps/place/" + la + "," + lon ;
        GetLine();
        }      
      s2 = "  🚨";
    ...
    if (flamewater == 1 && flame == 1) { //สั่งปิดน้ำ
      ...
      flamewater = 0;
      if (Line_stage == 1){
        message = "บ้านเลขที่ 493 ขณะนี้ไม่ตรวจพบเปลวไฟแล้ว www.google.com/maps/place/" + la + "," + lon ;
        GetLine();
        }         
      }        
  }     
...
void handleNewMessages(int numNewMessages){
  ...
  for (int i = 0; i < numNewMessages; i++){
    ...
    if (text == "/HELP") { 
        message = "บ้านเลขที่ 493 พบเหตุผิดปกติ ขอความช่วยเหลือด่วน www.google.com/maps/place/" + la + "," + lon ;
    ...
    if (bot.messages[i].longitude != 0 || bot.messages[i].latitude != 0) {     
      Firebase.setString(firebaseData, "/home2/Location/Longitude",String(bot.messages[i].longitude, 6));
      Firebase.setString(firebaseData, "/home2/Location/Latitude",String(bot.messages[i].latitude, 6));  
      if (Firebase.getString(firebaseData, "/home2/Location/Latitude")) {
        if (firebaseData.dataType() == "string") {
         la = firebaseData.stringData();
       }
